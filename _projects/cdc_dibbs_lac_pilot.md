...
{% capture summary %}
As part of the [Data Integration Building Blocks (DIBBs)](/work/experience/cdc-dibbs/) engagement with the Centers for Disease Control and Prevention (CDC), the Los Angeles County (LAC) Department of Public Health needed timely access to electronic case reporting (eCR) data to monitor and respond to disease outbreaks. Skylight worked with LAC and the CDC to develop a cutting-edge, modular data pipeline that enabled LAC to automatically process and enrich multiple data streams, including eCR data and electronic laboratory reporting (ELR), to improve downstream data analysis and case investigation.
{% endcapture %}
...
{% capture solution %}
**Building on a previous pilot** with the Virginia Department of Health, Skylight worked with LAC to improve its disease surveillance infrastructure as part of the CDC's broader data modernization initiative. The Skylight team developed and deployed a cutting-edge, modular data pipeline to automatically process and enrich multiple data streams, including COVID-19 eCR files and ELR. This open-source, cloud-based pipeline — composed of modular software components referred to as [Data Integration Building Blocks (DIBBs)](/work/experience/cdc-dibbs/) — significantly reduces the time it takes for LAC's disease surveillance teams to receive and act upon public health data, while also improving the quality of that data.

**Conducting comprehensive discovery research,** the DIBBs team understood eCR workflows, identified product support needs, and assessed the value of processing eCR data for LAC disease surveillance teams. **Engaging LAC staff throughout development,** the team participated in an iterative software development process with weekly agile ceremonies and regular product demonstrations to continuously refine the pipeline. **Performing rigorous testing,** the team conducted user acceptance testing with LAC staff to identify and mitigate barriers to adoption for the DIBBs pipeline. The team also compared the performance of LAC's pre-pilot data processing to data processing after the DIBBs pipeline was deployed to test record linkage performance and measure data quality, and evaluated how the pipeline affected the experience of case investigators that monitor and report on Hepatitis A to assess public health impact. **Enabling independent operations,** the DIBBs team developed a compendium of resources (the Handoff Hub) for LAC staff to use post-pilot that enables them to independently operate and customize the pipeline.
...
**Scaling solutions across jurisdictions,** the DIBBs team gained insights on how to use and adapt modular, open-source solutions to solve data challenges that are present in many public health jurisdictions. The issues present in LAC's disease surveillance infrastructure — siloed, low-quality data — are common across public health departments, making this work valuable for multiple disease surveillance systems.
{% endcapture %}
