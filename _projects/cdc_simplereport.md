...
title: Modernizing infectious disease reporting for under-resourced testing sites
...
{% include callout.html
  type = "pullquote"
  content = "It's a challenge to do the test, put it on paper, then go back and enter it in both [county and state] formats."
%}
...
On the receiving end, public health departments had to process manual reports sent from a variety of locations, which took time and resulted in errors. This cumbersome process made it difficult to act fast and stay informed.
{% endcapture %}

{% capture solution %}
To strengthen the infrastructure of COVID-19 testing and reporting, the CDC and U.S. Digital Service (USDS) launched a broader initiative to modernize public health data infrastructure. We've supported this work by helping to launch and grow [SimpleReport]({{ page.project_url }}), a free tool that makes it easy to record results for rapid disease testing.

SimpleReport is designed to be used as both a workflow and reporting tool for non-traditional and under-resourced test sites, regardless of whether or not they have a digital connection to a public health department's disease surveillance system. SimpleReport results are automatically sent to public health departments via a single point of connection called ReportStream, another tool developed through this initiative. As one interviewee described the reporting process:

{% include callout.html
  type = "pullquote"
  content = "Going from what we were doing to [SimpleReport] is awesome. It's very easy to use."
%}
...
After fully assuming SimpleReport management from our USDS partners, we've continued making the tool as easy to use as possible while expanding its functionality. We enabled scaling by redesigning the account creation, login, and password reset experiences, which sped up onboarding while maintaining security. We proved the ability to support other diseases by adding test result recording for multiplex devices, which test for both COVID-19 and flu. We also grew SimpleReport's reporting capabilities by adding a feature that allows users to upload test results in bulk using a CSV file.

To maintain high performance, we implemented automated service monitoring, worked closely with the CDC support team to resolve end-user issues, and provided 24/7 on-call engineering support. We practiced continuous integration and continuous deployment for secure and fast development — for example, when the code for a feature is tested and done, it's live in the application for users 20 minutes later. We also ensured SimpleReport meets high web accessibility standards by integrating reviews in design and engineering workflow.

As we continue to support ongoing COVID-19 reporting, we're also focusing on pandemic readiness and filling in gaps in other public health reporting. To further this goal, we've expanded SimpleReport for use with additional infectious diseases beyond COVID-19. We've worked with California's Department of Public Health to support the reporting of flu and respiratory syncytial virus (RSV) results from facilities who didn't have a way to report results electronically. We've established a pilot with the Los Angeles County Department of Public Health to provide a reporting solution for organizations conducting HIV and STI testing. And we've created and circulated a communication plan to current and new SimpleReport users to explain and publicize the new functionality for additional diseases.

In addition to reducing the burden for non-traditional and under-resourced facilities who test for conditions besides COVID-19, this work also ensures that SimpleReport has the flexibility necessary to be quickly deployed for the next pandemic. By building out and linking public health infrastructure where these connections didn't exist before, SimpleReport, along with other CDC data modernization tools, is fundamentally changing disease incidence reporting in the U.S. These tools are laying the structural groundwork for efficient and effective responses to future health crises.
{% endcapture %}

{% capture results %}
- Over 8.2M tests have been reported since launch (from January 2021 to April 2024)
...
- Following a successful pilot with California, support for flu reporting launched in July 2023 and RSV reporting went live in January 2024 — since then, more than 1,150 facilities in five states have reported over 206,000 flu test results and over 20,000 RSV test results
- Support for HIV reporting is scheduled to go live in 2025 with the Los Angeles Community Health Project
