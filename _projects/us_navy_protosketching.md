...
Each one-hour sprint translated user stories into a clickable mockup the team could test. Three sprints over an afternoon: the team wrote user stories capturing the Reserve's core mobile requirements, then developers immediately translated them into functional mock-ups. Using HTML, CSS, JavaScript, and Twitter Bootstrap for responsive layout, they created a mobile website that simulated the key functionality R2S needed — viewable on phones and laptops alike.
