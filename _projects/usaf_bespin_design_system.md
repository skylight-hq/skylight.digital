...
Off-the-shelf design systems offered a starting point but didn’t work for the Air Force without significant customization. No open source framework worked as-is without dark mode for nighttime operations, compliance with Web Content Accessibility Guidelines (WCAG) 2.0, and alignment with Air Force visual identity. Teams either adopted something that didn’t quite fit or built bespoke solutions that couldn’t be reused — and neither path closed the gap at the scale the portfolio demanded.
{% endcapture %}

{% capture solution %}
**Audits and interviews revealed the portfolio’s diversity demanded two design systems, not one.** Component inventories showed dozens of redundant implementations of the same elements — buttons, date pickers, data tables — built in different frameworks with different accessibility standards. But the products themselves ranged from dense enterprise tools to lightweight mobile utilities, running on different tech stacks with fundamentally different interaction needs. A single design system wouldn’t serve both ends.
